...
static rfbBool resize(rfbClient* client) {
  ...
  //printf("%i", client->width);
  //return -1;
  int width=864,height=480,
    depth=client->format.bitsPerPixel;

  client->updateRect.x = client->updateRect.y = 0;
  client->updateRect.w = 856; client->updateRect.h = height;
  //printf("%i", client->width);
  //return -1;
...
void Push_DRC_Frame(rfbClient *cl) {
  ...
  // push frame to DRC
    std::vector<drc::u8> pixels((drc::u8*)cl->frameBuffer,
                                ...
                                856 * drc::kScreenHeight * 4);
    g_streamer->PushVidFrame(&pixels, 856, drc::kScreenHeight,
  ...
  do {
    ...
    while(1) {
      ...
      */
      ...
      if (vncUpdate) {
        SDL_UpdateTexture(sdlTexture, NULL, cl->frameBuffer, 856 * 4);
